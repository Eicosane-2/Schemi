	...
	for (std::size_t i = 0; i < mesh_.surfacesSize(); ++i)
	{
		...
		{ 	//KT-solver
			...
			if ((SNeighbour >= 0) && (SOwner <= 0))
			{
				...
				numFluxes.rhokTurb.val()[i] = (SNeighbour
						* velocityProjectionOwner
						* surfaceOwnerSide.rhokTurb.cval()[i]
						- SOwner * velocityProjectionNeighbour
								* surfaceNeighbourSide.rhokTurb.cval()[i]
								...
								* surfaceOwnerSide.rhoepsTurb.cval()[i]
								...
								+ SNeighbour * SOwner
										* (surfaceNeighbourSide.rhoepsTurb.cval()[i]
												- surfaceOwnerSide.rhoepsTurb.cval()[i]))
								* denominator * mesh_.surfaces()[i].N();
				numFluxes.rhoaTurb.val()[i] = (SNeighbour
						* velocityProjectionOwner
						* surfaceOwnerSide.rhoaTurb.cval()[i]
						- SOwner * velocityProjectionNeighbour
								* surfaceNeighbourSide.rhoaTurb.cval()[i]
						+ SNeighbour * SOwner
								* (surfaceNeighbourSide.rhoaTurb.cval()[i]
										- surfaceOwnerSide.rhoaTurb.cval()[i]))
						* denominator * mesh_.surfaces()[i].N();
				numFluxes.rhobTurb.val()[i] = (SNeighbour
						* velocityProjectionOwner
						* surfaceOwnerSide.rhobTurb.cval()[i]
						- SOwner * velocityProjectionNeighbour
								* surfaceNeighbourSide.rhobTurb.cval()[i]
		...
		{	//HLLC-solver
			...
			if (SOwner >= 0)
			{
				...
				numFluxes.totalEnergy.val()[i] = (velocityProjectionOwner
						* (totalEnergyState + pressureState
								+ turbulentPressureOwner))
						* mesh_.surfaces()[i].N();

				numFluxes.rhokTurb.val()[i] = velocityProjectionOwner
				...
				numFluxes.rhoaTurb.val()[i] = rhoaState
						* velocityProjectionOwner * mesh_.surfaces()[i].N();
			...
			else if (Sc >= 0)
			{
				...
				const vector momentumState =
						(surfaceOwnerSide.momentum.cval()[i] * factor
								- mesh_.surfaces()[i].N()
										* (surfaceOwnerSide.pressure.cval()[i]
												+ turbulentPressureOwner
												- POwnNei)) * denominator;
			...
			else if (SNeighbour > 0)
			{
				...
				const scalar rhoEpsState =
						surfaceNeighbourSide.rhoepsTurb.cval()[i] * factor
								* denominator;
